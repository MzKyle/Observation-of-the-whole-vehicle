...
std::vector<Blade> Detector::Detect(cv::Mat & src_img)
{
  ...
  if (img.isContinuous()) { // 检查矩阵的数据是否在内存中是连续存储的
    img = img.reshape(1, 1);
  } else {
...
bool Detector::calibrate_kpts(Blade & blade, cv::Mat & img)
{
  ...
  //
  cv::circle(
    debug_img, cv::Point(255,255), 80, cv::Scalar(255, 255, 0), -1);
...
// void Detector::ProcessVideo(const std::string& video_path)
// {
//     cv::VideoCapture cap(video_path);

//     if (!cap.isOpened()) {
//         RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Could not open video file: %s", video_path.c_str());
//         return;
//     }

//     cv::Mat frame;
//     while (cap.read(frame)) {
//         std::vector<Blade> blades = Detect(frame);
//         draw_blade(frame);
//         std::cout << "Detected " << blade_array_.size() << " blades." << std::endl;
...
//     std::string model_path = "/home/lwg/Desktop/ros_2_ws/src/rm_buff-main/buff_detector/model";
//     std::string video_path = "/home/lwg/Desktop/buff_detector/example/first.mp4";

//     rm_buff::Detector detector(model_path);
