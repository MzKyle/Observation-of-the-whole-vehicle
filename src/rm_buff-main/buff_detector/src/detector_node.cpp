...
: Node("buff_detector", options)
{
  ...
  result_img_pub_ = image_transport::create_publisher(this, "/debug/buff_result_img");
  bin_img_pub_ = image_transport::create_publisher(this, "/detector/buff_bin_img");
  debug_img_pub_ = image_transport::create_publisher(this, "/detector/buff_debug_img");
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & image_msg)
{
  ...
  // if (!detector_->debug_img.empty()) {
  //   debug_img_pub_.publish(
  //     cv_bridge::CvImage(image_msg->header, "rgb8", detector_->debug_img).toImageMsg());
  // } else {
  //   RCLCPP_WARN(this->get_logger(), "Debug image is empty, skipping publication.");
  // }
