...
std::vector<Armor> Detector::detect(const cv::Mat & input)//检测装甲板
{
...
cv::Mat Detector::preprocessImage(const cv::Mat & rgb_img)//二值化图像
{
  cv::Mat gray_img;
...
std::vector<Light> Detector::findLights(const cv::Mat & rbg_img, const cv::Mat & binary_img)
{
  ...
  cv::findContours(binary_img, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);//查找二值图像中的轮廓
  ...
  for (const auto & contour : contours) {
    ...
    auto r_rect = cv::minAreaRect(contour);//计算轮廓的最小外接矩形，并创建一个 Light 对象
    auto light = Light(r_rect);

    if (isLight(light)) {
      auto rect = light.boundingRect();
      if (  // Avoid assertion failed 确保灯光区域在图像范围内
        0 <= rect.x && 0 <= rect.width && rect.x + rect.width <= rbg_img.cols && 0 <= rect.y &&
        0 <= rect.height && rect.y + rect.height <= rbg_img.rows) {
        int sum_r = 0, sum_b = 0;
        auto roi = rbg_img(rect); //计算灯光区域内红色像素和蓝色像素的总和，确定灯光的颜色
        ...
        lights.emplace_back(light);//将符合条件的灯光添加到 lights 向量中
      }
    }
  }
...
{
  // The ratio of light (short side / long side)灯光的短长比
  float ratio = light.width / light.length;
  bool ratio_ok = l.min_ratio < ratio && ratio < l.max_ratio;

  bool angle_ok = light.tilt_angle < l.max_angle;

  bool is_light = ratio_ok && angle_ok;//比例 角度都满足才满足
...
std::vector<Armor> Detector::matchLights(const std::vector<Light> & lights)
{
  ...
  for (auto light_1 = lights.begin(); light_1 != lights.end(); light_1++) { //双重循环遍历灯光
    for (auto light_2 = light_1 + 1; light_2 != lights.end(); light_2++) {
      if (light_1->color != detect_color || light_2->color != detect_color) continue;//如果两个灯光的颜色与检测颜色不匹配，则跳过当前组合

      if (containLight(*light_1, *light_2, lights)) {//包含关系检查
        continue;
      }

      //装甲板类型判断
      auto type = isArmor(*light_1, *light_2);//判断两个灯光是否可以组成一个有效的装甲板，如果可以，则获取装甲板的类型
...
bool Detector::containLight(
  const Light & light_1, const Light & light_2, const std::vector<Light> & lights)//获取灯光的顶点 将两个灯光的顶部和底部顶点存储在一个向量中
{
  auto points = std::vector<cv::Point2f>{light_1.top, light_1.bottom, light_2.top, light_2.bottom};
  auto bounding_rect = cv::boundingRect(points);//计算包含所有顶点的最小边界矩形

  for (const auto & test_light : lights) {//遍历所有灯光，跳过与当前两个灯光相同的灯光
    if (test_light.center == light_1.center || test_light.center == light_2.center) continue;

    if (//如果测试灯光的顶部、底部或中心点在边界矩形内，则返回 true，表示存在另一个灯光。
...
ArmorType Detector::isArmor(const Light & light_1, const Light & light_2)
{
  ...
  // Distance between the center of 2 lights (unit : light length)灯光中心距离
  ...
  // Angle of light center connection灯光连接角度
